...
export default function CloudTable({
  ...
  // Clear selection/menu when server results change (filters/page).
  useEffect(() => {
      ...
      <div className="border-b border-slate-200 bg-white dark:border-gray-600 dark:bg-gray-800">
        ...
        <Table.Toolbar>
          <Table.SearchInput
            ...
            ariaLabel="Search cloud plans"
            placeholder="Search cloud plans"
          />
        </Table.Toolbar>
      </div>
          ...
          {loading ? (
            ...
            plans.map((p) => (
              <Table.Row key={p._id}>
                ...
                <Table.Td className="uppercase">{p.type}</Table.Td>
                <Table.Td>{`INR ${Number(p.price ?? 0).toLocaleString()}/${p.period || 'month'}`}</Table.Td>
                <Table.Td>
