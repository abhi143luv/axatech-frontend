...
export default function AdminHeroBannerTable({
  items = [],
  loading = false,
  statusFilter = 'all',
  ...
  const openSingleDeleteConfirm = (item) => {
    closeMenu();
    setDeleteConfirm({ ids: [item._id], single: true, title: item.title });
  };
    ...
    <>
          ...
          {loading ? (
            ...
            items.map((item) => (
              <Table.Row key={item._id}>
                ...
                <Table.Td className="text-slate-600 dark:text-gray-400">
                  {item.placement === 'home' ? 'Homepage' : 'Licenses & TSS'}
                </Table.Td>
      ...
      {deleteConfirm && (
        <ConfirmModal
          ...
          title={deleteConfirm.single ? 'Delete banner' : 'Delete banners'}
          message={
            deleteConfirm.single
              ? `Are you sure you want to delete "${deleteConfirm.title}"? This cannot be undone.`
              : `Are you sure you want to delete ${deleteConfirm.ids.length} selected banner(s)? This cannot be undone.`
          }
          confirmLabel="Delete"
          cancelLabel="Cancel"
