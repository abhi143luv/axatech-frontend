...
const FALLBACK_HIGHLIGHT_ICONS = [CogLoopIcon, CheckCircleOutlineIcon, RocketIcon];

export default function TdlBusinessSpecific() {
  const [content, setContent] = useState(null);

  useEffect(() => {
    api.tdlBusinessSpecificContent
      .get()
      .then(setContent)
      .catch(console.error);
  }, []);
  ...
  const industryHighlights = Array.isArray(content.industryHighlights) ? content.industryHighlights : [];
  const points = Array.isArray(content.points) ? content.points : [];

  return (
    <>
      <PageMeta
        title={content.metaTitle || 'Business-Specific TDL | Axatech'}
        description={
          content.metaDescription || 'Industry-specific TDL solutions built for your unique business workflow in Tally.'
        }
      ...
      <section className="bg-linear-to-b from-slate-50 to-white pb-20 md:pb-24 dark:from-gray-900 dark:to-gray-900/90">
        <div className="hero-gradient-section py-20 md:py-24">
          ...
          <SectionHeader
            label={content.label || '5.5 Business-Specific TDL'}
            title={content.title || 'Customizations for Your Industry'}
            subtitle={content.subtitle || 'Purpose-built TDL enhancements designed around industry workflows.'}
            centered
            as="h1"
            inverse
        ...
        <div className="mx-auto mt-10 max-w-5xl px-5">
          ...
          <div className="mt-8">
            <Button
              ...
              className="px-6"
            >
              {content.ctaText || 'Request Business-Specific TDL'}
            </Button>
          </div>
        </div>
      </section>
