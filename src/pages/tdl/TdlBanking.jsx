...
export default function TdlBanking() {
  ...
  const bankingHighlights = Array.isArray(content.bankingHighlights) ? content.bankingHighlights : [];
  const points = Array.isArray(content.points) ? content.points : [];

  return (
    <>
      <PageMeta
        title={content.metaTitle || 'Banking TDL | Axatech'}
        description={content.metaDescription || 'Streamline bank workflows in Tally with custom banking TDL automation.'}
      ...
      <section className="bg-linear-to-b from-slate-50 to-white pb-20 md:pb-24 dark:from-gray-900 dark:to-gray-900/90">
        <div className="hero-gradient-section py-20 md:py-24">
          ...
          <SectionHeader
            label={content.label || '5.6 Banking TDL'}
            title={content.title || 'Streamline Banking Workflows in Tally'}
            subtitle={content.subtitle || 'Automate key banking operations and reduce manual effort.'}
            centered
            as="h1"
            inverse
        ...
        <div className="mx-auto mt-10 max-w-5xl px-5">
          <div data-aos="fade-up" className="grid gap-4 md:grid-cols-3">
            {bankingHighlights.map((highlight, idx) => {
          ...
          <div className="mt-8">
            <Button
              ...
              className="px-6"
            >
              {content.ctaText || 'Request Banking TDL'}
            </Button>
          </div>
        </div>
      </section>
