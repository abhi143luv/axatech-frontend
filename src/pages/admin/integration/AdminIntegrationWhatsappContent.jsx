...
export default function AdminIntegrationWhatsappContent() {
  ...
  useEffect(() => {
    api.admin.integrationWhatsappContent
      .get()
      .then(setContent)
      .catch(console.error);
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    setMessage('');
    try {
      await api.admin.integrationWhatsappContent.update(content);
      setMessage('Saved successfully.');
      toast.success('Integration WhatsApp content saved');
    } catch (err) {
  ...
  const handleReset = async () => {
    ...
    setMessage('');
    try {
      await api.admin.integrationWhatsappContent.remove();
      const fresh = await api.admin.integrationWhatsappContent.create(EMPTY_STATE);
      setContent(fresh);
      setMessage('Content reset to defaults.');
      toast.success('Integration WhatsApp content reset');
    } catch (err) {
    ...
    <div className="mx-auto max-w-[1280px]">
      <header className="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            type="submit"
            form="admin-integration-whatsapp-form"
            variant="primary"
            fullWidth={false}
            disabled={saving}
            loading={saving}
      ...
      <form id="admin-integration-whatsapp-form" onSubmit={handleSubmit} className="space-y-6">
