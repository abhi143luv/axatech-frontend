...
export default function AdminIntegrationThirdPartyContent() {
  ...
  useEffect(() => {
    api.admin.integrationThirdPartyContent
      .get()
      .then(setContent)
      .catch(console.error);
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    setMessage('');
    try {
      await api.admin.integrationThirdPartyContent.update(content);
      setMessage('Saved successfully.');
      toast.success('Integration Third Party content saved');
    } catch (err) {
  ...
  const handleReset = async () => {
    ...
    setMessage('');
    try {
      await api.admin.integrationThirdPartyContent.remove();
      const fresh = await api.admin.integrationThirdPartyContent.create(EMPTY_STATE);
      setContent(fresh);
      setMessage('Content reset to defaults.');
      toast.success('Integration Third Party content reset');
    } catch (err) {
    ...
    <div className="mx-auto max-w-[1280px]">
      <header className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <h1 className="m-0 text-2xl font-bold tracking-tight text-slate-800 dark:text-white">
          Integration Third Party Content
        </h1>
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            type="submit"
            form="admin-integration-third-party-form"
            variant="primary"
            fullWidth={false}
            disabled={saving}
            loading={saving}
      ...
      <form id="admin-integration-third-party-form" onSubmit={handleSubmit} className="space-y-6">
