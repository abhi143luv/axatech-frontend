...
export default function AdminIntegrationExcelImportContent() {
  ...
  useEffect(() => {
    api.admin.integrationExcelImportContent
      .get()
      .then(setContent)
      .catch(console.error);
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    setMessage('');
    try {
      await api.admin.integrationExcelImportContent.update(content);
      setMessage('Saved successfully.');
      toast.success('Integration Excel Import content saved');
    } catch (err) {
  ...
  const handleReset = async () => {
    ...
    setMessage('');
    try {
      await api.admin.integrationExcelImportContent.remove();
      const fresh = await api.admin.integrationExcelImportContent.create(EMPTY_STATE);
      setContent(fresh);
      setMessage('Content reset to defaults.');
      toast.success('Integration Excel Import content reset');
    } catch (err) {
    ...
    <div className="mx-auto max-w-[1280px]">
      <header className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <h1 className="m-0 text-2xl font-bold tracking-tight text-slate-800 dark:text-white">
          Integration Excel Import Content
        </h1>
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            type="submit"
            form="admin-integration-excel-import-form"
            variant="primary"
            fullWidth={false}
            disabled={saving}
            loading={saving}
      ...
      <form id="admin-integration-excel-import-form" onSubmit={handleSubmit} className="space-y-6">
