...
export default function AdminTdlBankingContent() {
  ...
  const handleChange = (e) => {
    ...
    if (name.startsWith('highlight_')) {
      ...
      setContent((c) => ({
        ...c,
        bankingHighlights:
          c.bankingHighlights?.map((item, j) => (j === i ? { ...item, [field]: value } : item)) || [],
      }));
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    setMessage('');
    try {
      await api.admin.tdlBankingContent.update(content);
      setMessage('Saved successfully.');
      toast.success('Tdl banking content saved');
    } catch (err) {
  ...
  const handleReset = async () => {
    ...
    setMessage('');
    try {
      await api.admin.tdlBankingContent.remove();
      const fresh = await api.admin.tdlBankingContent.create(DEFAULT_CREATE_BODY);
      setContent(fresh);
      setMessage('Content reset to defaults.');
      toast.success('Tdl banking content reset');
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-[1280px]">
      ...
      <form id="admin-tdl-banking-form" onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <h2 className="mb-3 text-base font-semibold text-slate-800 dark:text-gray-100">Banking Highlights</h2>
          <div className="space-y-4">
            {(content.bankingHighlights || []).map((highlight, i) => (
              <div key={i} className="rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-gray-700 dark:bg-gray-900/40">
                <Input label={`Highlight ${i + 1} title`} name={`highlight_${i}_title`} value={highlight.title || ''} onChange={handleChange} />
                <Input
                  label="Description"
