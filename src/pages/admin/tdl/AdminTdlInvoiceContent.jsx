...
export default function AdminTdlInvoiceContent() {
  ...
  const handleChange = (e) => {
    ...
    if (name.startsWith('highlight_')) {
      ...
      setContent((c) => ({
        ...c,
        invoiceHighlights:
          c.invoiceHighlights?.map((item, j) => (j === i ? { ...item, [field]: value } : item)) || [],
      }));
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    setMessage('');
    try {
      await api.admin.tdlInvoiceContent.update(content);
      setMessage('Saved successfully.');
      toast.success('Tdl invoice content saved');
    } catch (err) {
  ...
  const handleReset = async () => {
    ...
    setMessage('');
    try {
      await api.admin.tdlInvoiceContent.remove();
      const fresh = await api.admin.tdlInvoiceContent.create(DEFAULT_CREATE_BODY);
      setContent(fresh);
      setMessage('Content reset to defaults.');
      toast.success('Tdl invoice content reset');
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-[1280px]">
      ...
      <form id="admin-tdl-invoice-form" onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <h2 className="mb-3 text-base font-semibold text-slate-800 dark:text-gray-100">Invoice Highlights</h2>
          <div className="space-y-4">
            {(content.invoiceHighlights || []).map((highlight, i) => (
