...
export default function AdminTdlMisReportingContent() {
  ...
  useEffect(() => {
    api.admin.tdlMisReportingContent
      .get()
      .then(setContent)
      .catch(console.error);
  }, []);
  ...
  const handleChange = (e) => {
    ...
    if (name.startsWith('pillar_')) {
      ...
      setContent((c) => ({
        ...c,
        misReportingPillars:
          c.misReportingPillars?.map((item, j) => (j === i ? { ...item, [field]: value } : item)) ||
          [],
      }));
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.admin.tdlMisReportingContent.update(content);
      setMessage('Saved successfully.');
      toast.success('Tdl mis reporting content saved');
    } catch (err) {
  ...
  const handleReset = async () => {
    ...
    try {
      await api.admin.tdlMisReportingContent.remove();
      const fresh = await api.admin.tdlMisReportingContent.create(DEFAULT_CREATE_BODY);
      setContent(fresh);
      setMessage('Content reset to defaults.');
      toast.success('Tdl mis reporting content reset');
    } catch (err) {
    ...
    <div className="mx-auto max-w-[1280px]">
      <header className="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            type="submit"
            form="admin-tdl-mis-reporting-form"
            variant="primary"
            fullWidth={false}
            disabled={saving}
            loading={saving}
      ...
      <form id="admin-tdl-mis-reporting-form" onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <h2 className="mb-3 text-base font-semibold text-slate-800 dark:text-gray-100">Reporting Pillars</h2>
          <div className="space-y-4">
            {(content.misReportingPillars || []).map((pillar, i) => (
              <div
                key={i}
