...
export default function AdminTdlSecurityControlContent() {
  ...
  useEffect(() => {
    api.admin.tdlSecurityControlContent
      .get()
      .then(setContent)
      .catch(console.error);
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    setMessage('');
    try {
      await api.admin.tdlSecurityControlContent.update(content);
      setMessage('Saved successfully.');
      toast.success('Security Control TDL content saved');
    } catch (err) {
  ...
  const handleReset = async () => {
    ...
    setMessage('');
    try {
      await api.admin.tdlSecurityControlContent.remove();
      const fresh = await api.admin.tdlSecurityControlContent.create(DEFAULT_CREATE_BODY);
      setContent(fresh);
      setMessage('Content reset to defaults.');
      toast.success('Security Control TDL content reset');
    } catch (err) {
    ...
    <div className="mx-auto max-w-[1280px]">
      <header className="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            type="submit"
            form="admin-tdl-security-control-form"
            variant="primary"
            fullWidth={false}
            disabled={saving}
            loading={saving}
      ...
      <form id="admin-tdl-security-control-form" onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <h2 className="mb-3 text-base font-semibold text-slate-800 dark:text-gray-100">Security Pillars</h2>
          <div className="space-y-4">
            {(content.securityPillars || []).map((pillar, i) => (
              <div
                key={i}
