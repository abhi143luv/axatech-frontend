...
export default function AdminTdlBusinessSpecificContent() {
  ...
  useEffect(() => {
    api.admin.tdlBusinessSpecificContent
      .get()
      .then(setContent)
      .catch(console.error);
  }, []);
  ...
  const handleChange = (e) => {
    ...
    if (name.startsWith('highlight_')) {
      ...
      setContent((c) => ({
        ...c,
        industryHighlights:
          c.industryHighlights?.map((item, j) => (j === i ? { ...item, [field]: value } : item)) ||
          [],
      }));
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    setMessage('');
    try {
      await api.admin.tdlBusinessSpecificContent.update(content);
      setMessage('Saved successfully.');
      toast.success('Tdl business specific content saved');
    } catch (err) {
  ...
  const handleReset = async () => {
    ...
    setMessage('');
    try {
      await api.admin.tdlBusinessSpecificContent.remove();
      const fresh = await api.admin.tdlBusinessSpecificContent.create(DEFAULT_CREATE_BODY);
      setContent(fresh);
      setMessage('Content reset to defaults.');
      toast.success('Tdl business specific content reset');
    } catch (err) {
    ...
    <div className="mx-auto max-w-[1280px]">
      <header className="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            type="submit"
            form="admin-tdl-business-specific-form"
            variant="primary"
            fullWidth={false}
            disabled={saving}
            loading={saving}
      ...
      <form id="admin-tdl-business-specific-form" onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <h2 className="mb-3 text-base font-semibold text-slate-800 dark:text-gray-100">Industry Highlights</h2>
          <div className="space-y-4">
            {(content.industryHighlights || []).map((highlight, i) => (
              <div
                key={i}
