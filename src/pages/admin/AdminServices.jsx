  ...
  return (
    <div className="mx-auto max-w-[1280px]">
      <header className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <h1 className="m-0 text-2xl font-bold tracking-tight text-slate-800 dark:text-white">Services</h1>
      ...
      <div className="mb-6 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <table className="w-full border-collapse text-sm [&_tbody_tr:last-child_td]:border-b-0">
            <thead>
              <tr>
                ...
                <th className="border-b border-gray-200 bg-slate-50 px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:border-gray-600 dark:bg-gray-700/50 dark:text-slate-400">Slug</th>
            ...
            <tbody>
              {list.map((s) => (
                <tr key={s._id} className="transition-colors duration-150 hover:bg-slate-50 dark:hover:bg-gray-700/30">
                  {editing === s._id ? (
                    <td colSpan={4} className="border-b border-slate-100 px-4 py-3.5 align-middle dark:border-gray-600">
                      <div className="flex flex-wrap items-start gap-3 py-2">
                        <input className={`${inputClass} min-w-[120px] max-w-[200px]`} placeholder="Title" value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} />
                        <input className={`${inputClass} min-w-[120px] max-w-[200px]`} placeholder="Slug" value={form.slug} onChange={(e) => setForm((f) => ({ ...f, slug: e.target.value }))} />
                        <textarea className={`${textareaClass} min-h-[60px] min-w-[200px] max-w-[320px]`} placeholder="Short description" value={form.shortDescription} onChange={(e) => setForm((f) => ({ ...f, shortDescription: e.target.value }))} rows={2} />
                        <textarea className={`${textareaClass} min-h-[60px] min-w-[200px] max-w-[320px]`} placeholder="Description" value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} rows={3} />
                        ...
                        <div className="flex w-full flex-wrap gap-2 border-t border-dashed border-gray-200 pt-2 dark:border-gray-600">
                          ...
                          <Button type="button" variant="error" fullWidth={false} onClick={() => remove(s._id)}>Delete</Button>
                        </div>
                      </div>
                    </td>
                  ) : (
                    <>
                      <td className="border-b border-slate-100 px-4 py-3.5 align-middle text-slate-700 dark:border-gray-600 dark:text-gray-300">{s.title}</td>
                      <td className="border-b border-slate-100 px-4 py-3.5 align-middle dark:border-gray-600">
                        <code className="rounded bg-slate-100 px-2 py-0.5 text-sm text-slate-600 dark:bg-gray-600 dark:text-gray-300">{s.slug}</code>
                      </td>
                      <td className="border-b border-slate-100 px-4 py-3.5 align-middle dark:border-gray-600">
                        <span className={s.isActive !== false ? 'inline-block rounded-md bg-success/15 px-2.5 py-1 text-[0.78rem] font-semibold uppercase tracking-wide text-success-dark dark:bg-success/20 dark:text-success-light' : 'inline-block rounded-md bg-slate-100 px-2.5 py-1 text-[0.78rem] font-semibold uppercase tracking-wide text-slate-500 dark:bg-gray-600 dark:text-gray-400'}>{s.isActive !== false ? 'Yes' : 'No'}</span>
                      </td>
        ...
        {list.length === 0 && (
          <div className="mx-4 mb-4 py-10 text-center text-base text-slate-500 dark:text-gray-400">No services yet. Add one to get started.</div>
        )}
      </div>
      ...
      {editing === 'new' && (
        <div className="fixed inset-0 z-1000 flex animate-admin-fadeIn items-center justify-center bg-slate-900/50 p-6" onClick={() => setEditing(null)}>
          <div className="flex max-h-[90vh] w-full max-w-[720px] flex-col overflow-hidden rounded-xl bg-white shadow-2xl animate-admin-slideUp dark:bg-gray-800" onClick={(e) => e.stopPropagation()}>
            <div className="shrink-0 border-b border-gray-200 px-6 py-5 text-lg font-bold text-slate-800 dark:border-gray-600 dark:text-white">Add Service</div>
              ...
              <div className="flex flex-col gap-4">
                <div className="grid grid-cols-2 gap-4 max-[520px]:grid-cols-1">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input className={inputClass} placeholder="Optional" value={form.slug} onChange={(e) => setForm((f) => ({ ...f, slug: e.target.value }))} />
                  </div>
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-medium text-slate-600 dark:text-gray-400">Short description</label>
                  <textarea className={textareaClass} value={form.shortDescription} onChange={(e) => setForm((f) => ({ ...f, shortDescription: e.target.value }))} rows={2} />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-medium text-slate-600 dark:text-gray-400">Description</label>
                  <textarea className={textareaClass} value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} rows={4} />
                </div>
