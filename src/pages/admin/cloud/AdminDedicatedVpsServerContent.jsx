...
export default function AdminDedicatedVpsServerContent() {
  ...
  useEffect(() => {
    api.admin.dedicatedVpsServerContent
      .get()
      .then(setContent)
      .catch(console.error);
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    setMessage('');
    try {
      await api.admin.dedicatedVpsServerContent.update(content);
      setMessage('Saved successfully.');
      toast.success('Dedicated VPS content saved');
    } catch (err) {
  ...
  const handleReset = async () => {
    ...
    setMessage('');
    try {
      await api.admin.dedicatedVpsServerContent.remove();
      const fresh = await api.admin.dedicatedVpsServerContent.create(EMPTY_STATE);
      setContent(fresh);
      setMessage('Content reset to defaults.');
      toast.success('Dedicated VPS content reset');
    } catch (err) {
  ...
  const featuresValue = Array.isArray(content.features) ? content.features.join('\n') : '';
  const plansValue = Array.isArray(content.plans) ? content.plans.join('\n') : '';

  return (
    <div className="mx-auto max-w-[1280px]">
      <header className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <h1 className="m-0 text-2xl font-bold tracking-tight text-slate-800 dark:text-white">
          Dedicated VPS Server Content
        </h1>
      ...
      <form id="admin-dedicated-vps-form" onSubmit={handleSubmit} className="space-y-6">
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <Input label="Meta title" name="metaTitle" type="text" value={content.metaTitle || ''} onChange={handleChange} />
            <Input
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <Input label="Subtitle" name="subtitle" type="text" value={content.subtitle || ''} onChange={handleChange} />
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <h2 className="mb-3 text-base font-semibold text-slate-800 dark:text-gray-100">Core Features</h2>
          <Input label="Features (one per line)" name="featuresText" type="textarea" value={featuresValue} onChange={handleFeaturesChange} />
        </section>

        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <h2 className="mb-3 text-base font-semibold text-slate-800 dark:text-gray-100">VPS Options</h2>
          <Input label="Plans (one per line)" name="plansText" type="textarea" value={plansValue} onChange={handlePlansChange} />
        </section>
